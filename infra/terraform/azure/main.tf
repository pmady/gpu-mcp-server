...
#      so a single untainted node runs the whole stack (operator, CoreDNS and
#      gpu-mcp-server) — the cheapest layout and a direct mirror of the EKS
...
resource "azurerm_kubernetes_cluster" "this" {
  ...
  default_node_pool {
    ...
    # Azure has no arm64 NVIDIA GPU VM sizes, so unlike the AWS sibling (dual
    ...
    # RuntimeClass). This is NVIDIA's documented AKS path and gives the
    # gpu-mcp-server chart everything it needs unchanged. See gpu_operator.tf.
    gpu_driver = "None"
    ...
    # NOTE: intentionally the (untainted) default/system pool. The GPU
    # operator controllers and CoreDNS schedule on the GPU node alongside
